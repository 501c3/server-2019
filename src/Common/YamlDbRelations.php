...
class YamlDbRelations extends YamlDbModel
{
    ...
    /**
     * YamlDbRelations constructor.
     * @param EventDispatcherInterface $dispatcher
     */
    public function __construct(EntityManagerInterface $entityManager, EventDispatcherInterface $dispatcher = null)
    {
        $this->teamPersonCalculator = new GeorgiaDanceSport();
    ...
    public function buildSoloTeam($description,$year,$partnerProficiencies,string $position) {
        ...
        $teamSolo=$this->teamPersonCalculator->buildTeamSolo($description,$year);
    ...
    public function buildCoupleTeam(array $description, int $year,array $partnerProficiencies,string $position)
    {
        ...
        /*@var array */
        $combinationList=$this->teamPersonCalculator->buildTeamPersons($description,$year,$partnerProficiencies);
        foreach($combinationList as $combination){
            $team = $combination['team'];
            $persons = $combination['persons'];
            /** @var Team $teamObj */
            $teamObj = $this->team[$team['type']]
                                [$team['status']]
                                [$team['sex']]
                                [$team['age']]
                                [$team['proficiency']];
